...
function toAuditLog(row: AuditLogRow): AuditLog {
 ...
 return {
  ...
  action: row.action,
  actor: (metadata.actor as AuditLog["actor"]) ?? "system",
  ...
  payload: typeof metadata.payload === "object" && metadata.payload !== null ? (metadata.payload as Record<string, unknown>) : metadata,
